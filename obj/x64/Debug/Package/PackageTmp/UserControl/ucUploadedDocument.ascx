...
<div>    
    <fieldset>
        ...
        <ol>
            <li>
                <asp:GridView ID="grddocument" CssClass="footable" CellPadding="5" AutoGenerateColumns="False" AllowPaging="True" PageSize="2" ToolTip="Document's Information" runat="server" OnPageIndexChanging="grddocument_PageIndexChanging" BorderColor="#CACACA" BorderStyle="Solid" BorderWidth="1px" >
                     <PagerStyle CssClass="gridBorderStyle pagination pagination-centered"/>
                    <Columns>
     ...
     <script type="text/javascript">
         $(function () {
             $('#<%=grddocument.ClientID %>').bind('footable_breakpoint', function () {
                $('#<%=grddocument.ClientID %>').trigger('footable_expand_first_row');
            }).footable();
        });
    </script>
</div>
